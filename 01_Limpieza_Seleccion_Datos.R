...
VHB_aguda_n <- hcc_fil %>% count(HBeAg) #1,5% positivo
ggplot(VHB_aguda_n, aes(x = HBeAg, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn( colours = terrain.colors(6))
  
VHB_cronica_n <- hcc_fil %>% count(HBsAg) #9%
ggplot(VHB_cronica_n, aes(x = HBsAg, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn( colours = terrain.colors(6))

VHC_n <- hcc_fil %>% count(HCVAb) #20%
ggplot(VHC_n, aes(x = HCVAb, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn(colours = terrain.colors(6))
                        
alcohol_n <- hcc_fil %>% count(Alcohol) #72%
ggplot(alcohol_n, aes(x = Alcohol, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn(colours = terrain.colors(6))
                        
HIV_n <- hcc_fil %>% count(HIV) #1,5%
ggplot(HIV_n, aes(x = HIV, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn(colours = terrain.colors(6))

NASH_n <- hcc_fil %>% count(NASH) #5%
ggplot(NASH_n, aes(x = NASH, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn(colours = terrain.colors(6))

sintomas_n <- hcc_fil %>% count(Symptoms) #70%
ggplot(sintomas_n, aes(x = Symptoms, y= n)) +
  geom_bar(stat = "identity") +
  scale_fill_gradientn(colours = terrain.colors(6))

cirrhosis_n <- hcc_fil %>% count(Cirrhosis) #90%
...
formula_02 <- dbinom(GGT ~ I(ALT^2), data = SA)
